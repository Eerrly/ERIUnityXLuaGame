...
public class BetterList<T>
{
	...
	void AllocateMore ()
	{
        T[] newList = (buffer != null) ? new T[Mathf.Max(buffer.Length << 1, minBufferSize)] : new T[minBufferSize];
    ...
    void CheckNeedAllocateMore(int appendCount)
    {
        ...
        {
            int count = Mathf.Max(size + appendCount, minBufferSize);
    ...
    {
        int bufferSize = Mathf.Max(minBufferSize, count);
